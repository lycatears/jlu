...
const form = useForm({
  validationSchema: toTypedSchema(z.object({
    uname: z.string().min(1, '请输入用户名'),
    password: z.string().min(1, '请输入密码'),
    remember: z.boolean(),
  })),
  initialValues: {
    uname: props.uname ?? localStorage.getItem('login_uname') ?? '',
    password: '',
    remember: !!localStorage.getItem('login_uname'),
  },
})
const onSubmit = form.handleSubmit((values) => {
  ...
  userStore.login(values).then(() => {
    if (values.remember) {
      localStorage.setItem('login_uname', values.uname)
    }
    else {
      localStorage.removeItem('login_uname')
    }
    emits('onLogin', values.uname)
  }).catch(() => {
...
// function testuname(uname: string) {
    ...
    <div v-show="type === 'default'">
      <form @submit="onSubmit">
        ...
        <div class="mb-4 flex-center-between">
          ...
          <FaButton variant="link" class="h-auto p-0" type="button" @click="emits('onResetPassword', form.values.uname)">
            忘记密码了?
          </FaButton>
        </div>
        <FaButton :loading="loading" size="lg" class="w-full" type="button" @click="onSubmit">
          登录
        </FaButton>
        <div class="mt-4 flex-center gap-2 text-sm">
          <span class="text-secondary-foreground op-50">还没有帐号?</span>
          <FaButton variant="link" class="h-auto p-0" type="button" @click="emits('onRegister', form.values.uname)">
            注册新帐号
          </FaButton>
        </div>
      </form>
      <!-- <div class="mt-4 text-center -mb-4">
        ...
        <div class="space-x-2">
          <FaButton variant="default" size="sm" plain @click="testuname('admin')">
            admin
          </FaButton>
          <FaButton variant="outline" size="sm" plain @click="testuname('test')">
            test
          </FaButton>
        </div>
      </div> -->
