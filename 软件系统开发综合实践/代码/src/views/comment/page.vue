...
function exportTable() {
  const header = ['评论ID', '用户ID', '用户名', '图书ID', '图书名称', '楼层号', '评论内容', '用户评分', '发表时间']
  const rows = tableData.value.map(row => [row.commentId, row.userId, row.uname, row.bookId, row.bookTitle, row.floorId, row.context, row.rating, row.createTime])
  const csvContent
    = `\uFEFF${// 防止中文乱码
