...
function exportTable() {
  const header = ['编号', '图书ID', '图书名称', '用户ID', '用户名', '借阅时间', '归还时间']
  const rows = tableData.value.map(row => [row.recordId, row.bookId, row.bookTitle, row.userId, row.uname, row.borrowDate, row.returnDate])
